...
const ProductSectionTriathlon = () => {
  ...
  // Pricing tables for different frame types - Triathlon specific sizes
                ...
                <div className="flex gap-3">
                  {typeOptions.map((option) => (
                    <div key={option.value} className="flex items-center space-x-2">
                      <RadioGroupItem value={option.value} id={`triathlon-${option.value}`} />
                      <Label htmlFor={`triathlon-${option.value}`} className="cursor-pointer">
                        {option.label}
                      </Label>
                    </div>
                  ))}
...
export default ProductSectionTriathlon;
