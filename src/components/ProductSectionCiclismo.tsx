// TODO: Atualizar preços específicos para modalidade ciclismo
// TODO: Atualizar imagens específicas para modalidade ciclismo
...
const ProductSectionCiclismo = () => {
  ...
  // Pricing tables for different frame types (PLACEHOLDER - mesmos valores de corrida)
  ...
  const handleAddToCart = () => {
    const productName = selectedType === "caixa-alta" ? "Quadro Caixa Alta - Ciclismo" : "Quadro Caixa Baixa - Ciclismo";
                ...
                <div className="flex gap-3">
                  {typeOptions.map((option) => (
                    <div key={option.value} className="flex items-center space-x-2">
                      <RadioGroupItem value={option.value} id={`ciclismo-${option.value}`} />
                      <Label htmlFor={`ciclismo-${option.value}`} className="cursor-pointer">
                        {option.label}
                      </Label>
                    </div>
                  ))}
