...
interface GalleryCarouselMobileProps {
  images: GalleryImage[];
  initialIndex?: number;
}

const GalleryCarouselMobile = ({ images, initialIndex = 0 }: GalleryCarouselMobileProps) => {
    ...
    <>
      <div style={{ width: "100%", padding: "0 16px", boxSizing: "border-box" }}>
        <div style={{ position: "relative", width: "100%" }}>
          ...
          <button
            onClick={() => move(-1)}
            style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", zIndex: 10, background: "rgba(255,255,255,0.95)", border: "1px solid rgba(0,0,0,0.15)", borderRadius: "50%", width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}
          >
            <ChevronLeft size={18} />
          </button>
          <button
            onClick={() => move(1)}
            style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", zIndex: 10, background: "rgba(255,255,255,0.95)", border: "1px solid rgba(0,0,0,0.15)", borderRadius: "50%", width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}
          >
...
export
cat > ~/projetos/onetouch3d-site-v2/src/components/GalleryCarouselMobile.tsx << 'ENDOFFILE'
import { useState, useEffect } from "react";
import { Dialog, DialogContent } from "@/components/ui/dialog";
import { X, ChevronLeft, ChevronRight } from "lucide-react";
