...
const ProductSectionViagemLocal = () => {
  ...
  // IMAGEM ESPECÍFICA DA LOJA VIAGEM
  const productImage = "/images/moldura-premium-viagem.webp";
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {sizeOptions.map((option) => (
                      <div key={option.size} className="flex items-center space-x-2">
                        ...
                        <Label htmlFor={`viagem-size-${option.size}`} className="cursor-pointer text-sm">
                          {option.size}
                        </Label>
                      </div>
                    ))}
