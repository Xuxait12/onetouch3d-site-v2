...
const ProductSectionViagemLocal = () => {
  ...
  useEffect(() => { localStorage.setItem('lastStorePage', 'viagem'); }, []);

  const colorOptions = [{ value: "preta-branca", label: "Preta/branca" }];
  const ALL_SIZES = ["33x43", "37x48", "43x53", "43x63", "53x73", "63x83", "83x103"];
  const availablePricedSizes = getAvailableSizes("caixa_alta");
  const displaySizes = ALL_SIZES.filter(size => availablePricedSizes.includes(size) || (QUOTE_SIZES.includes(size) && isQuoteOnly(size, "caixa_alta")));
  const finalDisplaySizes = ALL_SIZES.filter(size => availablePricedSizes.includes(size) || QUOTE_SIZES.includes(size));
  ...
  const getCurrentImage = () => productImages[selectedSize] || productImages["33x43cm"];
  ...
  const handleAddToCart = () => {
    ...
    addItem({ nome: productName, cor: colorDisplay, tamanho: selectedSize, quantidade: 1, precoUnitario: fullPrice, imagem: productImage, modalidade_id: MODALIDADES.viagem, tamanho_id: priceInfo.tamanho_id, tipo_moldura_id: priceInfo.tipo_moldura_id });
        ...
        <div className="rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-8" style={{ background: 'transparent !important' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-start">
            <div className="space-y-6 sm:space-y-8">
              <div className="relative"><img src={productImage} alt="Moldura Premium para viagem" className="w-full rounded-lg shadow-lg" loading="lazy" /></div>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div>
                ...
                {pricesLoading ? (<div className="grid grid-cols-2 gap-2 sm:gap-3">{[1,2,3,4].map(i => <Skeleton key={i} className="h-10 w-full" />)}</div>) : (
                  <RadioGroup value={selectedSize} onValueChange={setSelectedSize}><div className="grid grid-cols-2 gap-2 sm:gap-3">{finalDisplaySizes.map((size) => (<label key={size} htmlFor={`viagem-size-${size}cm`} className="flex items-center space-x-2 p-2 -m-1 cursor-pointer touch-manipulation rounded-md hover:bg-muted/50 active:bg-muted transition-colors select-none" onTouchEnd={(e) => { e.preventDefault(); setSelectedSize(size + "cm"); }}><RadioGroupItem value={size + "cm"} id={`viagem-size-${size}cm`} className="pointer-events-none" /><span className="text-xs sm:text-sm">{size}cm</span></label>))}</div></RadioGroup>
                )}
              </div>
              ...
              {pricesLoading ? (<div className="bg-muted/50 p-4 rounded-lg space-y-2"><Skeleton className="h-4 w-32" /><Skeleton className="h-8 w-40" /><Skeleton className="h-4 w-48" /></div>) : (
                <div className="bg-muted/50 p-4 rounded-lg">
                  {isQuote ? (<div className="text-2xl font-bold text-primary">Solicitar orçamento</div>) : (<><div className="text-sm text-muted-foreground mb-1">Preço:</div><div className="text-3xl font-bold text-green-600 mb-2">R$ {fullPrice.toFixed(2).replace('.', ',')}</div><InstallmentsPreview amount={fullPrice} /></>)}
                </div>
              )}
              {isQuote ? (
